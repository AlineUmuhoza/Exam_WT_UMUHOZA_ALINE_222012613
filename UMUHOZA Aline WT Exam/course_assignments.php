...
<head>
  ...
  <title>course_assignments</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
    ...
    <h1><u> course_assignments Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="aid">assignment_id :</label>
        ...
        <input type="text" id="ast" name="ast" required><br><br>

        <label for="asm">assignment_marks:</label>
        <input type="text" id="asm" name="asm" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO course_assignments(assignment_id , id, course_id , assignment_title, assignment_marks) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("iiiss", $aid, $uid, $cid, $ast, $asm);
...
// SQL query to fetch data from the course_assignments table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of course_assignments</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of course_assignments</h2></center>
    <table border="5">
        <tr>
            <th>assignment_id</th>
            <th>user_id</th>
     ...
     include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all course_assignments
        $sql = "SELECT * FROM course_assignments";
        $result = $connection->query($sql);

        // Check if there are any course_assignments
        if ($result->num_rows > 0) {                       
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $aid = $row['assignment_id']; // Fetch the assignment_id
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_course_assignments.php?assignment_id=$aid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_course_assignments.php?assignment_id=$aid'>Update</a></td> 
                </tr>";
            }
        } else {
