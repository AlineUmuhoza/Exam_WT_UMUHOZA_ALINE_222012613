...
<section>
    ...
    <h1><u> course_materials Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="mid">material_id:</label>
        <input type="number" id="mid" name="Pid"><br><br>

        <label for="cid">course_id :</label>
        <input type="number" id="cid" name="cid"><br><br>

        <label for="mty">material_type:</label>
        <input type="text" id="mty" name="mty" required><br><br>

        <label for=mtt>material_title:</label>
        <input type="text" id="mtt" name="mtt" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO course_materials(material_id, course_id, material_type, material_title) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("iiss", $mid, $cid, $mty, $mtt);
...
// SQL query to fetch data from the course_materials table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of course_materials</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of course_materials</h2></center>
    <table border="5">
        <tr>
            <th>material_id</th>
            <th>course_id</th>
      ...
      include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all course_materials
        $sql = "SELECT * FROM course_materials";
        $result = $connection->query($sql);
                                                   
        // Check if there are any course_materials
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $mid = $row['material_id']; // Fetch the material_id
                echo "<tr>
                    <td>" . $row['material_id'] . "</td>
                    <td>" . $row['course_id'] . "</td>
                    <td>" . $row['material_type'] . "</td>
                    <td>" . $row['material_title'] . "</td>
                    <td><a style='padding:4px' href='delete_course_materials.php?material_id=$mid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_course_materials.php?PaymentID=$mid'>Update</a></td> 
                </tr>";
            }
        } else {
