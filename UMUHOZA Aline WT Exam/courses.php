...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO courses(course_id, instructor_id, title, start_date, end_date, price) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("iisssd", $cid, $iid, $tit, $std, $end, $prc);
...
if ($result->num_rows > 0) {                                  
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr>
            ...
            <td>" . $row['end_date'] . "</td>
            <td>" . $row['price'] . "</td>
            <td><a style='padding:4px' href='delete_courses.php?course_id=$cid'>Delete</a></td> 
            <td><a style='padding:4px' href='update_courses.php?course_id=$cid'>Update</a></td> 
        </tr>";
