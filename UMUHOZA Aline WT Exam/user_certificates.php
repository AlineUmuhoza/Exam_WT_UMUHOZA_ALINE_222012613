...
<section>
    ...
    <h1><u> user_certificates Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="ceid">certificate_id:</label>
        <input type="number" id="ceid" name="ceid"><br><br>
        ...
        <label for=idt>issue_date:</label>
        <input type="date" id="idt" name="idt" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO user_certificates(certificate_id, id, course_id, issue_date) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("iiis", $ceid, $uid, $cid, $idt);
...
// SQL query to fetch data from the user_certificates table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of user_certificates</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of user_certificates</h2></center>
    <table border="5">
        <tr>
      ...
      include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all user_certificates
        $sql = "SELECT * FROM user_certificates";
        $result = $connection->query($sql);

        // Check if there are any user_certificates
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $ceid = $row['certificate_id']; // Fetch the certificate_id
                echo "<tr>
                    <td>" . $row['certificate_id'] . "</td>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['course_id'] . "</td>
                    <td>" . $row['issue_date'] . "</td>
                    <td><a style='padding:4px' href='delete_user_certificates.php?certificate_id=$ceid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_user_certificates.php?certificate_id=$ceid'>Update</a></td> 
                </tr>";
            }
        } else {
