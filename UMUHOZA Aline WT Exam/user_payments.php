...
<section>
    ...
    <h1><u> user_payments Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="paid">payment_id:</label>
        ...
        <input type="date" id="pdt" name="pdt" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO user_payments(payment_id, id, course_id , amount, payment_date) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("iiiss", $paid, $uid, $cid, $amnt, $pdt);
...
// SQL query to fetch data from the user_payments table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of user_payments</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of user_payments</h2></center>
    <table border="5">
        <tr>
            <th>payment_id</th>
            <th>user_id</th>
     ...
     include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all user_payments
        $sql = "SELECT * FROM user_payments";
        $result = $connection->query($sql);

        // Check if there are any user_payments
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_user_payments.php?payment_id=$paid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_user_payments.php?payment_id=$paid'>Update</a></td> 
                </tr>";
            }
        } else {
